...
TEST(fft, sin) {
    ...
    const auto out = fft<direction::time_to_freq>(std::span{ in });
    ...
    write_test_data("fft_sin", std::span{ in }, std::span{ normalized_out });
}
...
TEST(fft, cos) {
    ...
    const auto normalized_out = normalize<double>(out);
    write_test_data("fft_cos", std::span{ in }, std::span{ normalized_out });
}
...
TEST(fft, identity) {
    ...
    const auto normalized_out = normalize<double>(out);
    write_test_data("fft_identity", std::span{ in }, std::span{ normalized_out });
}

TEST(fft, harmonicSum) {
    ...
    const auto normalized_out = normalize<double>(out);
    write_test_data("fft_harmonic_sum", std::span{ in }, std::span{ normalized_out });
}
...
TEST(fft, random) {
    ...
    const auto normalized_out = normalize<double>(out);
    write_test_data("fft_random", std::span{ in }, std::span{ normalized_out });
}
